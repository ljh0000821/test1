        ...
        <template v-if="!twrId">
            <u-sticky>
                <view class="tabs-group flex-around">
                    ...
                    <ef-select-btn width="140rpx" :data="TroTypeLevel" type="select" label="text" id="id" placeholder="等级" @change="levelChange"></ef-select-btn>
                ...
                <view class="list-item" v-for="item in listData" :key="item.id" @click="toDetails(item)">
                    <view class="flex-between">
                        <view class="flex-start flex1">
                            ...
                            <text class="list-item-status m-l-16">外力</text>
                            <!-- <view class="m-l-16 gray-text ">
                                <img src="../../../../static/common/ic_add_ins_tower.png" alt="" srcset="">
                                <text>{{item.twrCode}}</text>
                            </view> -->
                            <text class="flex1 gray-text m-l-16 text-ellipsis">{{item.troReport|clearLineFeed}}</text>
                        </view>
...
import { troextList } from "@/api/hiddenDanger/index";
...
export default {
    ...
    methods: {
        ...
        _getTypelist() {
            ...
            // });
            //隐患等级
            this.$store.dispatch("getList", "getTroTypeLevel1").then((res) => {
        ...
        toDetails(data) {
            ...
            if (data.state != 1) {
                uni.navigateTo({
                    url: "pages/task/hiddenDanger/details?type=0&id=" + data.id
                });
            } else {
                uni.navigateTo({
                    url:
                        "pages/task/hiddenDanger/addDanger?type=edit&activeTabs=0&id=" +
                        data.id
                });
            }
        }
