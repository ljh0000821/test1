<template>
    <view class="view-modal" v-if="showModalView" @tap.stop.self="dateCancelAction" @touchmove.stop.prevent="() => {}" :class="[modalAnimation?'ani_start':'ani_end']">
        <view class="date-view" :class="[modalAnimation?'ani-top':'ani-btm']">
            <view class="head-view">
                <view class="cancel-view">
                    <text @click="dateCancelAction">取消</text>

                </view>
                <view class="title-view">
                    <text class="m-l-24" @click="resetAction">重置</text>
                </view>
...
export default {
    data() {
        ...
        if (this.viewMode == 1) {
            ...
            dateProp = [
                parseInt(this.dateFormat(this.startDate, "hh")),
                parseInt(this.dateFormat(this.startDate, "mm"))
            ];
        }
        ...
        dualData: {
            handler: function (hd) {
                ...
                if (hd.mode == "date") {
                    this.viewMode = 1;
                    let date = new Date(
                        this.dateIndex == 0 ? hd.startDate : hd.endDate
                    );
                    if (date) {
                        const year = date.getFullYear();
                        const month = date.getMonth() + 1;
                    ...
                    this.viewMode = 2;
                    const salue = this.hhmmFormat(
                        this.dateIndex == 0 ? hd.startDate : hd.endDate
                    );
                    if (salue) {
                        this.datevalue = [
                            parseInt(this.dateFormat(salue, "hh")),
                            parseInt(this.dateFormat(salue, "mm"))
                        ];
                    }
    ...
    methods: {
        ...
        dateOkAction: function () {
            if (this.passageEnd == null || this.passageEnd.length == 0) {
            ...
            } else if (this.endTime && this.passageEnd > this.endTime) {
                let tips =
                    this.endTime.slice(0, 10) == getNowTime().slice(0, 10)
                        ? "当前日期"
                        : this.endTime;
                uni.showToast({
                    title: "结束时间不能大于" + tips,
                ...
                let data = [
                    this.passageStart + " 00:00:00",
                    this.passageEnd + " 23:59:59"
                ];
                if (data[0] > data[1]) {
                    uni.showToast({
                        title: "开始时间不能大于结束时间",
                ...
                if (this.viewMode == 2) {
                    data[0] = this.dateFormat(data.start, "hh:mm");
                    data[1] = this.dateFormat(data.end, "hh:mm");
                }
                setTimeout(() => {
                    self.showModalView = false;
        ...
        dateAction: function (e) {
            ...
            if (e == 0) {
                const date = new Date(
                    Date.parse(this.passageStart.replace(/-/g, "/"))
                );
                if (this.viewMode == 1) {
                    const year = date.getFullYear();
                    const month = date.getMonth() + 1;
