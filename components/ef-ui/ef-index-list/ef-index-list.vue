<template>
    ...
    <view>
        <view class="u-index-bar">
            <slot />
            <view v-if="showSidebar" class="u-index-bar__sidebar" @touchstart.stop.prevent="onTouchMove" @touchmove.stop.prevent="onTouchMove" @touchend.stop.prevent="onTouchStop" @touchcancel.stop.prevent="onTouchStop">
                <view v-for="(item, index) in indexList" :key="index" class="u-index-bar__index" :style="{zIndex: zIndex + 1, color: activeAnchorIndex === index ? activeColor : ''}" :data-index="index">
                    {{ item }}
                </view>
            </view>
    ...
    methods: {
        ...
        getActiveAnchorIndex() {
            ...
            for (let i = this.children.length - 1; i >= 0; i--) {
                if (this.scrollTop >= children[i].$el.offsetTop) {
                    return i;
                }
            }
            return -1;
        ...
        onTouchMove(event) {
            //this.sidebar.height 520
            // this.children.length 26
            this.touchmove = true;
            const sidebarLength = this.children.length;
            const touch = event.touches[0];
            const itemHeight = this.sidebar.height / sidebarLength; //20
            let clientY = 0;
            clientY = touch.clientY; //361       
            let top = (window.innerHeight - this.sidebar.height) / 2; //20
            let index = Math.floor((clientY - top) / itemHeight);
            if (index < 0) {
                index = 0;
        ...
        scrollToAnchor(index) {
            ...
            if (anchor) {
                this.$emit("select", anchor.index, anchor.$el.offsetTop);
            }
        }
    }
};
