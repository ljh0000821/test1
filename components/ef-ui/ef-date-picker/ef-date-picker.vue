<template>
    <view class="gpp-datePicker" @click.stop="show">
        <slot />
        <view class="g-dp-mask" :class="{'show':pipkerShowFlag}" @click.stop="onCancel" @touchmove.stop.prevent catchtouchmove="true"></view>
...
export default {
    ...
    methods: {
        ...
        propsCheck() {
            ...
            let now = new Date(this.defaultValue.replace(/-/g, "/")).getTime();
        ...
        onConfirm() {
            this.pipkerShowFlag = false;
            this.$emit("onConfirm", {
                dateValue: this.getDateValue(this.selectedValue) + " 00:00:00",
                dateValueIndex: this.selectedValue
            });
        },
